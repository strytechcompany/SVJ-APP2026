...
export default function OldBalanceCustomerList({ navigation }) {
  ...
  const fetchCustomers = useCallback(async () => {
    try {
      const res = await balanceSettlementAPI.getOldBalanceCustomers();
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      ...
      <View style={styles.balanceRow}>
        <Text style={styles.balanceLabel}>Current Old Balance</Text>
        <Text style={styles.balanceValue}>{formatBalanceForCustomer(item.oldBalance, item)}</Text>
      </View>
      <View style={styles.actionsRow}>
        <TouchableOpacity
          style={styles.historyBtn}
          onPress={() => navigation.navigate('OldBalanceHistory', { customerId: item._id, customerName: item.customerName })}
        >
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('OldBalanceSettlement', { customerId: item._id })}
        >
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.header, { paddingTop: topPad }]}>
        ...
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle}>Old Balance</Text>
        </View>
        <TouchableOpacity style={styles.headerBtn} onPress={() => navigation.navigate('OldBalanceSettlementHistory')}>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptyText}>No customers with an Old Balance</Text>
            </View>
          }
        />
      )}
...
const styles = StyleSheet.create({
  ...
  balanceRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: '#FDECEA', borderRadius: 10, padding: 12, marginTop: 12 },
  balanceLabel: { fontSize: 12, color: '#8A6B3C', fontWeight: '600' },
  balanceValue: { fontSize: 16, color: '#D32F2F', fontWeight: '800' },
