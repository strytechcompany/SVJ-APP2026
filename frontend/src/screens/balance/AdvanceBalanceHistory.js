...
// Read-only Advance Balance settlement history for a single customer.
export default function AdvanceBalanceHistory({ route, navigation }) {
  ...
  const fetchHistory = useCallback(async () => {
    try {
      const res = await balanceSettlementAPI.getSettlementsByCustomer(customerId, 'ADVANCE');
    ...
    return (
      <View style={styles.card}>
        ...
        <View style={styles.row}>
          ...
          <Text style={[styles.value, { color: item.finalAdvanceBalance > 0 ? '#2E7D32' : '#D32F2F', fontWeight: '800' }]}>
            {formatBalanceForCustomer(item.finalAdvanceBalance > 0 ? item.finalAdvanceBalance : item.finalOldBalance, item.customerId)} ({item.finalAdvanceBalance > 0 ? 'Advance' : 'Old'})
          </Text>
        </View>
      </View>
    );
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.header, { paddingTop: topPad }]}>
        ...
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle}>Advance Balance History</Text>
