...
// - previewPayload (unsaved): came straight from AdvanceBalanceSettlement's
//   "Preview" button. Nothing has been written to MongoDB yet — "Save Bill"
//   is the only action that creates the settlement record.
// - settlementId (already saved): came from Advance Balance Settlement
//   History — loads the exact saved bill from MongoDB, never regenerates it.
//   Save Bill is pre-marked "Saved" since it already exists.
export default function AdvanceBalanceSettlementPreview({ route, navigation }) {
  ...
  const {
    ...
    previousAdvanceBalance, finalOldBalance, finalAdvanceBalance,
  ...
  const handlePrintBill = async () => {
    ...
    try {
      ...
      if (!settlementId) navigation.navigate('AdvanceBalanceSettlementHistory');
    } catch (e) {
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.row}><Text style={styles.label}>Current Advance Balance:</Text><Text style={[styles.value, { color: '#2E7D32', fontSize: 15 }]}>{formatBalanceForCustomer(previousAdvanceBalance, customerForUnit)}</Text></View>
        </View>
        ...
        <View style={styles.summaryBox}>
          ...
          <View style={styles.divider} />
          <View style={styles.summaryRow}><Text style={styles.summaryLabel}>Current Advance Balance</Text><Text style={styles.summaryValue}>{formatBalanceForCustomer(previousAdvanceBalance, customerForUnit)}</Text></View>
          <View style={styles.divider} />
          <View style={styles.summaryRow}>
            ...
            <Text style={[styles.summaryValue, { color: finalAdvanceBalance > 0 ? '#2E7D32' : '#D32F2F' }]}>
              {formatBalanceForCustomer(finalAdvanceBalance > 0 ? finalAdvanceBalance : finalOldBalance, customerForUnit)}
            </Text>
          </View>
        </View>
