...
export default function OldBalanceSettlement({ route, navigation }) {
  ...
  const rawFinal = safeNumber(previousOldBalance - totalSettlementGram);
  const finalOldBalance = rawFinal < 0 ? 0 : rawFinal;
  const finalAdvanceBalance = rawFinal < 0 ? safeNumber(previousAdvanceBalance + Math.abs(rawFinal)) : previousAdvanceBalance;
  ...
  const handlePreview = () => {
    ...
    navigation.navigate('OldBalanceSettlementPreview', { previewPayload });
  };
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.header, { paddingTop: topPad }]}>
        ...
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle}>Old Balance Settlement</Text>
        </View>
        <View style={{ width: 40 }} />
      </View>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.section}>
          ...
          <View style={styles.row}><Text style={styles.label}>Current Old Balance:</Text><Text style={[styles.value, { color: '#D32F2F', fontSize: 15 }]}>{formatBalanceForCustomer(previousOldBalance, customer)}</Text></View>
        </View>
        ...
        <View style={styles.summaryBox}>
          ...
          <View style={styles.divider} />
          <View style={styles.summaryRow}><Text style={styles.summaryLabel}>Current Old Balance</Text><Text style={styles.summaryValue}>{formatBalanceForCustomer(previousOldBalance, customer)}</Text></View>
          <View style={styles.divider} />
          <View style={styles.summaryRow}>
            ...
            <Text style={[styles.summaryValue, { color: finalOldBalance > 0 ? '#D32F2F' : '#2E7D32' }]}>
              {formatBalanceForCustomer(finalOldBalance > 0 ? finalOldBalance : finalAdvanceBalance, customer)}
            </Text>
          </View>
        </View>
