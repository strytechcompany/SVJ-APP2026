...
// Global Old Balance Settlement History — every saved settlement bill across
// all customers, newest first. Populated only when a bill is Saved/Printed
// from the Settlement Preview screen.
export default function OldBalanceSettlementHistory({ navigation }) {
  ...
  const fetchHistory = useCallback(async () => {
    try {
      const res = await balanceSettlementAPI.getHistory('OLD');
  ...
  const renderItem = ({ item }) => {
    ...
    const currentIsOld = item.finalOldBalance > 0;
    const currentBalanceValue = currentIsOld ? item.finalOldBalance : item.finalAdvanceBalance;
    return (
      <TouchableOpacity
        style={styles.card}
        activeOpacity={0.8}
        onPress={() => navigation.navigate('OldBalanceSettlementPreview', { settlementId: item._id })}
      >
        ...
        <View style={styles.row}>
          ...
          <Text style={[styles.value, { color: currentIsOld ? '#D32F2F' : '#2E7D32', fontWeight: '800' }]}>
            {formatBalanceForCustomer(currentBalanceValue, customer)} ({currentIsOld ? 'Old' : 'Advance'})
          </Text>
        </View>
        <View style={styles.divider} />
    ...
    <View style={styles.container}>
      ...
      <View style={[styles.header, { paddingTop: topPad }]}>
        ...
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle}>Old Balance Settlement History</Text>
        </View>
        <View style={{ width: 40 }} />
      </View>
